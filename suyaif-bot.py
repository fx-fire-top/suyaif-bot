...
# লগিং সিস্টেম আরও শক্তিশালী করা হলো বটের ইন্টারনাল ট্র্যাকিংয়ের জন্য
logging.basicConfig(
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', 
    level=logging.INFO,
    handlers=[logging.StreamHandler(sys.stdout)]
)
logger = logging.getLogger("SuyaifMegaBot")

load_dotenv()

# [১০০% ফিক্সড] তোমার নতুন দেওয়া বটের একদম ফ্রেশ এপিআই টোকেন
BOT_TOKEN = "8919947592:AAERZVMUSS5VA2FqtBlvE3xBDO9_E62LcOY"

# রেন্ডার এপিআই সেটিংস
...
# অ্যান্টি-স্প্যাম ও সিকিউরিটি ডাটাবেস
user_last_message_time = defaultdict(float)
user_spam_count = defaultdict(int)
SPAM_LIMIT = 1.0  # মেসেজের মাঝের সেকেন্ড গ্যাপ
BAN_TIME = 10     # স্প্যাম করলে কত সেকেন্ড বট রেসপন্স করা বন্ধ রাখবে

# ওনার ব্যানার আল্ট্রা-বোল্ড ফিক্সড
OWNER_BANNER = """
<b><code>███╗   ███╗██████╗     ███████╗██╗   ██╗██╗  ██╗██████╗ ██████╗ 
████╗ ████║██╔══██╗    ██╔════╝██║   ██║╚██╗██╔╝██╔══██╗██╔═══╝ 
██╔████╔██║██║  ██║    ███████╗██║   ██║ ╚███╔╝ ███████║█████╗  
...
def deploy_render_server() -> bool:
    ...
    try:
        ...
        logger.error(f"Render API Redeploy Error: {e}")
        return False
...
async def restart_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if deploy_render_server():
        await status_msg.edit_text("🚀 <b>সফল হয়েছে বন্ধু!</b> রেন্ডার সার্ভারে (Clear Cache & Deploy) সফলভাবে শুরু হয়েছে। ১-২ মিনিটের মধ্যে বট সম্পূর্ণ ফ্রেশ হয়ে রি-অনлайн হবে।")
    else:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "refresh_menu":
            ...
            await query.edit_message_text(text="🛡️ <b>ফায়ারওয়াল স্ক্যানিং...</b>\n\nবটের ইন্টারনাল টোকেন ও অ্যান্টি-হ্যাকিং প্রোটেকশন ১০০% নিরাপদ রয়েছে। Ends Clear.", parse_mode='HTML')
    except Exception as e:
        logger.error(f"Callback Button Error: {e}")
...
async def handle_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # অ্যাডভান্সড অ্যান্টি-স্প্যাম মেকানিজম (হ্যাকিং ও ক্র্যাশ থেকে বাঁচতে)
    if current_time - user_last_message_time[uid] < SPAM_LIMIT:
        user_spam_count[uid] += 1
        if user_spam_count[uid] > 3:
            await update.message.reply_text(f"⚠️ <b>স্প্যামিং এলার্ট!</b> আপনি খুব দ্রুত রিকোয়েস্ট পাঠাচ্ছেন। বট পরবর্তী {BAN_TIME} সেকেন্ডের জন্য আপনার মেসেজ ইগনোর করবে।")
            user_last_message_time[uid] = current_time + BAN_TIME
    ...
    if not step:
        # ওনার ডিটেকশন সিস্টেম (বাংলা ও ইংরেজি মিলিয়ে সব কিওয়ার্ড)
        owner_keywords = ["মালিক কে", "মালিক", "owner", "creator", "তৈরি করছে", "সুয়াইф", "সোয়াইব", "suyaif", "shoyaif", "soyaif"]
        ...
        if any(x in user_raw for x in ["সালাম", "salam", "assalamualaikum", "আসসালামু আলাইকুম"]):
            await update.message.reply_text("📿 ওয়াআলাইকুম আসসালাম ওয়া রাহমাতুল্লাহ! সোয়ائب ভাইয়ের মেগা এআই বটে আপনাকে स्वागतম। আপনি কেমন আছেন? 🥰")
            return
        ...
        # রিয়ালিস্টিক আল্ট্রা-এআই চ্যাট রেসপন্স (ইমোজি সহ র্যান্ডমাইজড ফ্লেক্সিবল মেসেজ)
        ai_replies = [
            ...
            f"🎯 <b>সিস্টেম অ্যানালাইসিস:</b> আপনি অনেক চমৎকার কথা বলেছেন বন্ধু! সোয়াইফ ভাইয়ের অল-ইন-ওয়ান বটের স্পেশাল মেনুগুলো দেখতে নিচের /start কম্যান্ডটি দিন। ⚡💎"
        ]
        await update.message.reply_text(random.choice(ai_replies), parse_mode='HTML')
        return

    # বাটন ক্লিকের পরবর্তী প্রসেসিং ইনপুটসমূহ
    try:
        if step == "wait_emojis":
            mixes = ["🔥☠️🔥", "❤️‍🔥🥺❤️‍🔥", "👾👽👾", "👻🔥👻", "🦅👑🦅", "💥⚡💥", "🎭✨🎭"]
            ...
            # বাটন ছাড়া সম্পূর্ণ আলাদা মেসেজে আকর্ষণীয় করে নাম জেনারেট করা
            await update.message.reply_text("✨ <b>আপনার নামের ৫টি আল্ট্রา-বোল্ড ডিজাইন রেডি করা হয়েছে:</b>\n<i>(যেকোনো একটি লাইনের ওপর জাস্ট টাচ করলেই অটো কপি হয়ে যাবে)</i>", parse_mode='HTML')
        ...
        logger.error(f"Error handling step inputs: {e}")
        context.user_data.clear()

if __name__ == "__main__":
    # গ্লোবাল ক্র্যাশ লুপ প্রোটেকশন চালু করা হলো
    while True:
        try:
            logger.info("Initializing Application Builder with Anti-Crash Shield...")
            ...
            logger.info(f"[{BOT_NAME}] SYSTEM RUNNING ON MAX SECURE LEVEL...")
            app.run_polling(close_loop=False)
        except Exception as crash_error:
            logger.critical(f"Bot System crashed due to: {crash_error}. Auto-restarting session in 3 seconds...")
            time.sleep(3)
